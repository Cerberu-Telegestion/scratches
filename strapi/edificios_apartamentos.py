...
STRAPI_URL_LOCATIONS = STRAPI_URL + 'long-stays-locations'
...
print('Cargando datos de apartamentos.json...')
with open('apartamentos.json') as f:
...
def subir_edificio(edificio):
    ...
    data = {
        'data': {
            ...
            'neighborhood': edificio['distrito_barrio'],
...
def subir_apartamento(apartamento_key, apartamento, edificio_id):
    ...
    data = {
        'data': {
            ...
            'area': float(apartamento['Metros cuadrados'].replace(',', '.')) if apartamento['Metros cuadrados'] else None,
            'coliving': apartamento['Piso compartido'] == 'TRUE',
        }
    }
...
# Subir todos los edificios y sus apartamentos asociados
for edificio in edificios:
    edificio_id = subir_edificio(edificio)
    if edificio_id:
        for apartamento_key, apartamento in apartamentos.items():
            if apartamento.get('Edificio al que pertenece') == edificio['nombre']:
                subir_apartamento(apartamento_key, apartamento, edificio_id)
    else:
        print(f'Error al subir el edificio: {edificio["nombre"]}')