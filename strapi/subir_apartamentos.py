...
with open('solo_apartamentos.json') as f:
...
def subir_apartamento(apartamento_key, apartamento):
    print(f'Subiendo apartamento: {apartamento_key}')
    data = {
        'data': {
            'name': apartamento_key,
            'externalId': apartamento['identificador'],
            'description': apartamento['Descripción'],
            'long_stays_location': 59,
...
# Subir todos los apartamentos asociados
for apartamento_key, apartamento in apartamentos.items():
    subir_apartamento(apartamento_key, apartamento)